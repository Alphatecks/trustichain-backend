...
exports.looksLikeTransactionId = looksLikeTransactionId;
exports.couldBeHexBlob = couldBeHexBlob;
exports.couldBeJsonTransaction = couldBeJsonTransaction;
exports.validateSignedTransactionFormat = validateSignedTransactionFormat;
/**
 ...
 */
function couldBeJsonTransaction(value) {
 ...
 */
function validateSignedTransactionFormat(value) {
    ...
    if (typeof value === 'string' && value.length < 50) {
        return {
            valid: false,
            error: "Invalid signed transaction format. Transaction blob appears too short (".concat(value.length, " characters). Expected a signed transaction from MetaMask/XRPL Snap (hex string 1000+ chars or transaction object)."),
            detectedFormat: 'unknown',
        };
    }
    ...
    return {
        valid: false,
        error: "Invalid signed transaction format. Expected a signed transaction from MetaMask/XRPL Snap (hex string, transaction object, or wrapped format like { tx_blob: \"...\" }). Got: ".concat(typeof value === 'string' ? value.substring(0, 100) + '...' : JSON.stringify(value).substring(0, 100) + '...'),
        detectedFormat: 'unknown',
    };
}
